...
echo "🚀 Installing Proteksi By @SyahrilxXx…"
echo "🕒 Timestamp: $TS"
echo
        ...
        if ($user) {
            if ($user->id !== 1 && $user->id !== 2) {
                ...
                if ((int) $ownerId !== (int) $user->id) {
                    throw new DisplayException('Gausah Hapus Server Orang Kocak 😂 @SyahrilxXx');
                }
            }
        }
...
class ServerController extends ClientApiController
{
    ...
    public function index(GetServerRequest $request, Server $server): array
    {
        ...
        if ($authUser->id !== 1 && (int) $server->owner_id !== (int) $authUser->id) {
            abort(403, '𝗟𝘂 𝗡𝗴𝗮𝗽𝗮𝗶𝗻 𝗞𝗼𝗰𝗮𝗸? 𝗚𝗮𝘂𝘀𝗮𝗵 𝗡𝗴𝗶𝗻𝘁𝗶𝗽 😂.');
        }
...
class IndexController extends Controller
{
    ...
    public function index(): View
    {
        $user = Auth::user();
        if (!$user || ($user->id !== 1 && $user->id !== 2)) {
            abort(403, '𝙇𝙪 𝙉𝙜𝙖𝙥𝙖𝙞𝙣 𝙆𝙤𝙘𝙖𝙠? 😂 @SyahrilxXx');
        }
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        $user = Auth::user();
        if (!$user || ($user->id !== 1 && $user->id !== 2)) {
            abort(403, '𝗟𝘂 𝗚𝗮𝗯𝗼𝗹𝗲𝗵 𝗔𝗸𝘀𝗲𝘀 𝗡𝗲𝘀𝘁 𝗠𝗲𝗻𝘂 𝗞𝗼𝗰𝗮𝗸 😂 @SyahrilxXx.');
        }
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        $user = Auth::user();
        if (!$user || ($user->id !== 1 && $user->id !== 2)) {
            abort(403, '𝙎𝙞𝙠𝙤𝙘𝙖𝙠 𝙈𝙖𝙪 𝙊𝙩𝙖𝙠 𝘼𝙩𝙞𝙠 𝙉𝙤𝙙𝙚😂 @SyahrilxXx');
        }
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        ...
        if ($request->user()->id !== 1) {
            throw new DisplayException("𝙈𝙖𝙪 𝘿𝙚𝙡𝙚𝙩𝙚 𝙐𝙨𝙚𝙧 𝙊𝙧𝙖𝙣𝙜? 😂 @SyahrilxXx!");
        }
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restricted as $field) {
            if ($request->filled($field) && $request->user()->id !== 1 && $request->user()->id !== 2) {
                throw new DisplayException("𝙋𝙚𝙢𝙞𝙡𝙞𝙠 𝙑𝙥𝙨 𝘿𝙤𝙖𝙣𝙜 𝙔𝙖𝙣𝙜 𝘽𝙞𝙨𝙖 𝙮𝙖𝙝𝙠 😂.");
            }
        }

        if ($user->root_admin && $request->user()->id !== 1 && $request->user()->id !== 2) {
            throw new DisplayException("𝙔𝙖𝙚𝙡𝙖𝙝 𝙂𝙖𝙪𝙨𝙖𝙝 𝙐𝙣𝙖𝙙𝙢𝙞𝙣 𝙊𝙧𝙖𝙣𝙜 😂, @SyahrilxXx.");
        }
